...
To test the USB cam, type "sudo ./FrameRates" (USB cams are recognized first by the Bus Manager?)
To test the Ethernet cam, type "./FrameRates" (The USB cam won't be recognized if "sudo" isn't used)
...
int runFrame(CameraBase& cam, string dir, int ms) {
    ...
    // Otherwise the first pictures comes out 
        //with the same frame value as the old value
    sleep(4);
    ...
    //take 3 images at this certain frame rate
    ...
    {                
        ...
        printf( "Grabbed image %d with a %d fps frame rate.  \n", imageCnt, ms );
...
int runSingleCamera(PGRGuid guid, CameraBase& cam, InterfaceType interfaceType) {
    ...
    //collect ms frame rates

    int start;
    int end;
    int inc;

    cout << "Enter the number of fps to start at (min=1) (must be integer): \n";
    cin >> start;

    cout << "Enter the number of fps to end at (max=40?) (must be integer): \n";
    cin >> end;

    cout << "Enter the number of fps to increment by (must be integer): \n";
    cin >> inc;


    int framevals[1000]; // 1000 is overkill
    int count = 0;
    ...
    // run pictures for each of those frame values
    for(int n=0; n<count; n++) {
        runFrame(cam, path, framevals[n]);
    }
